...
<div class="col-10 bd-content">
	...
	<div align="center">
		<c:if test="${startPage > PAGE_PER_BLOCK}">
			<button type="button" class="btn btn-primary" onclick="location.href='boardMyComment.do?account_num=${account_num }&pageNum=${startPage - 1}'">이전</button>
		</c:if>
		<c:forEach var="i" begin="${startPage}" end="${endPage}">
			<button type="button" class="btn btn-primary" onclick="location.href='boardMyComment.do?account_num=${account_num }&pageNum=${i}'">${i }</button>
		</c:forEach>
		<c:if test="${endPage < totalPage}">
			<button type="button" class="btn btn-primary" onclick="location.href='boardMyComment.do?account_num=${account_num }&pageNum=${endPage + 1}'">다음</button>
		</c:if>
	</div>
	</div>
	</div>
